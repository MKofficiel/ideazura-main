...
export async function POST(request: NextRequest, response: NextResponse) {
  ...
  const { data, error } = await resend.emails.send({
    from: "Ideazura <hello@ideazura.com>", // domaine Resend vérifié
    to: [email],
    subject: "You're on the Ideazura waitlist 🚀",
    reply_to: "mkdevcoding@gmail.com", // ton mail perso pour questions
    html: await render(
      WelcomeTemplate({ userFirstname: firstname || "there" }),
    ),
  });
